...
"""CoNLL 2000 shared task format support."""
...
# Number of sentences to output per document.
...
def is_document_break(line, sentences):
    # There are no document breaks in the data, instead split totally
    # arbitrarily.
    return (len(sentences) % SENTENCES_PER_DOCUMENT) == 0
...
def _parse_conll(source):
    ...
    for ln, line in enumerate(source):
        ...
        if is_sentence_break(line):
            ...
            current = []
            if is_document_break(line, sentences):
                yield sentences
                sentences = []
            continue
